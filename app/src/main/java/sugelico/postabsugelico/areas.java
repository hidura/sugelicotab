...
public class areas extends Fragment {
    ...
    public void onTrackSelected(UserProfile profile,
                                HashMap<Integer, Area> area,
                                cmpDetails companyProfile, HashMap<Long, CartProds> shoppingCart) {

        RecyclerView recycler =((RecyclerView)getActivity().findViewById(R.id.areas));
        recycler.setAdapter(new Area_adapter(getActivity(), profile, area, companyProfile, shoppingCart));

        GridLayoutManager manager = new GridLayoutManager(getContext(),1); // MAX NUMBER OF SPACES
        recycler.setLayoutManager(manager);
        ((ProgressBar)getActivity().findViewById(R.id.progressBararea4)).setVisibility(View.GONE);
